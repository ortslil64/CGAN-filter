...
import numpy as np
from deepbayesianfilter.components.components import downsample, upsample, downsample1d, upsample1d, discriminator_loss, generator_loss, sampling
...
class VariationalDeepFilter():
    def __init__(self, input_shape, output_shape, lr = 2e-4, n0_filters = 64, max_filters = 512, n0_filter_zise = 4, apply_dropout = True):
        ...
        z_mean = tf.keras.layers.Dense(x.shape[1])(x)
        x = sampling(z_mean)
        x = tf.keras.layers.Dense(shape_x[1]*shape_x[2]*shape_x[3],  activation=tf.nn.relu)(x)
        x = tf.keras.layers.Reshape(target_shape=tuple(shape_x[1:]))(x)
