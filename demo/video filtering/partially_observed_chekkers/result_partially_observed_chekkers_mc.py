...
for mc_run in range(mc_runs):
    ...
    x_test = x[mc_run*2:100 + mc_run*2]
    z_test = z[mc_run*2:100 + mc_run*2]
    pf = ParticleFilter_deep(Np = 10000,
                            No = 1,
                            ref_img = ref_img,
                            radiuses = [18],
                            initial_pose = [[24 + mc_run*2,18 + mc_run*4]],
                            beta = 1,
                            likelihood=df)
    ...
    for t in range(0+hist,len(x_test)-1):   
        z_new = z_test[t].copy() 
    ...
    img_name = "samples"+str(mc_run)+".png"
    matplotlib.image.imsave(img_name, full_img, cmap='gray')
    
    # ---- Saves a video ---- #  
    video_name = "samples"+str(mc_run)+".mp4"
    ...
    mc_df.append(img_err_df)
    mc_pf.append(img_err_pf)
    mc_direct.append(img_err_direct)
    # ---- Save error statistics ---- #
    
scipy.io.savemat('partially_observed_chekkers_data_mc.mat', mdict={'mc_df': np.array(mc_df),
                                                                 'mc_pf': np.array(mc_pf),
                                                                 'mc_direct': np.array(mc_direct)})
